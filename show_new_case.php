...
$conn = new mysqli('localhost','root','','covid');
 ...
 $query = "select * from new_cases ";
    ...
    echo "<td>".$cat['age']."</td>";
    echo "<td>".$cat['city']."</td>";
    echo "<td>".$cat['date_case']."</td>";
     echo"<td><button style='background-color: red;color: white'><a href='?id=".$cat['id']."'>Delete</a></button></td>";
            echo "<td><button style='background-color: blue;color: white,border:none'><a href='update_new_case.php?id=".$cat['id']."'>Update</a></button></td>";
        echo "</tr>";
 }	
 echo "</table>";
  if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $del = "delete from new_cases where id = '".$id."'";
...
<br><br>
<form method="post">
    <button name="back" style="border-radius: 5">back</button>
</form></div>
</body>
</html>
